...
{	
	...
	for(var i=0; i<this.particles.length; i++ )
	{
		...
		ctx.drawImage(this.texture, origin.x * scale.x, origin.y * scale.y, p.size.x, p.size.y);
		ctx.restore();		
	}
}
...
function createPowerupBar_Particle(particleTexture, posX, posY)
{
	var e = new Emitter(particleTexture, posX, posY);
	e.emissionSize.set(330, 0);
	e.emissionRate = 250.0;
	e.minLife = 0.5;
	e.maxLife = 1.0;
	e.transparency = 0.20;
	e.minVelocity.x = 0.0;
